...
bool search_dir_entry(struct partition* part, struct dir* pdir, const char* name, struct dir_entry* dir_e) {
    ...
    while (block_idx < block_cnt) {
        ...
        // 需要恢复p_de指向buf
        memset(buf, 0, SECTOR_SIZE);            // 将buf清零 下次再用
...
void dir_close(struct dir* dir) {
    ...
    // 2 root_dir所在的目录是低端1MB之内 并非在堆中 free会出问题
    if (dir == &root_dir) {
...
// 将目录项p_de写入父目录parent_dir中 io_buf由主函数提供
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
    ...
    // 将该目录的所有扇区地址存入all_blocks 12 + 128
    uint8_t block_idx = 0;
    ...
    while (block_idx < 140) {
        ...
        if (all_blocks[block_idx] == 0) {
            ...
            if (block_idx < 12) {
                ...
                // 若是尚未分配的一级间接块表(block_idx等于12表示第0块间接地址为0)
                // 将上面分配的块作为一级起始地址
        ...
        // 若第block_idx已存在 将其读进内存 然后在该块中查找空目录项
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
    ...
    bool is_dir_first_block = false;            // 目录的第一块
    ...
    while (block_idx < 140) {
        ...
        while (dir_entry_idx < dir_entrys_per_sec) {
            ...
            if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
                ...
                if (!strcmp((dir_e + dir_entry_idx)->filename, ".")) {
                    ...
                    // 统计这个扇区的目录项总个数 用来判断删除目录后是否回收该扇区
                    dir_entry_cnt ++;
        ...
        // 在此扇区中找到目录项后 清除该目录并判断是否回收扇区 随后退出循环直接返回
        ...
        if (dir_entry_cnt == 1 && !is_dir_first_block) {
            ...
            if (block_idx < 12) {
                ...
                // 在一级间接索引表中去除该间接块地址 
                // 先判断一级间接索引表中间接块的数量 如果只有一个间接块 连同间接索引表所在的块一同回收
    ...
    // 如果未找到返回flase 出现这种情况应该是serch_file出错
    return false;
