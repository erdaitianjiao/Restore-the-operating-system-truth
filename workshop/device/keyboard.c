...
static void intr_keyboard_handler(void) {
    ...
    break_code = ((scancode & 0x0080) != 0);        // 获取断码

    if (break_code) {
        // 如果是断码
        
        // 由于ctrl_r和ctrl_r都是两个字节 所以可用下面的方法获取make_code多字节的扫描码暂不处理
        uint16_t make_code = (scancode &= 0xff7f);

        // 若是下面三个键弹起了 则将状态设为false
    ...
    // 若为通码 只处理数组总定义的按键和alt_right和ctrl键就行 全是make_code
    else if ((scancode > 0x00 && scancode < 0x3b) || 
             (scancode == alt_r_make) || 
             (scancode == ctrl_r_make)) {
        
        // 判断是否与shift组合 用于索引以为数组里对应的字符
        bool shift = false;
        ...
        // 将扫秒的高字节置0 主要针对高字节是e0的扫描码

        char cur_char = keymap[index][shift];       // 在数组中找打对应的字符
