...
// 用于记录总拓展分区的起始lba 初始为0 partition_scan时以此为标记
int32_t ext_lba_base = 0;

uint8_t p_no = 0, l_no = 0;                     // 用于记录磁盘主分区和逻辑分区的下标

struct list partition_list;                     // 构建一个分区队列
...
struct boot_sector {
    ...
    struct partition_table_entry partition_table[4];    
                                                // 分区表中有四项 共64字节                                             
    uint16_t signature;                         // 启动扇区的结束标志是0x55 0xa
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    while (secs_done < sec_cnt) {
        ...
        // 在硬盘已经开始工作(开始在内部读数据或写数据)后才能阻塞自己现在硬盘已经开始忙了 
    ...
    while (part_idx ++ < 4) {
        ...
        if (p->fs_type == 0x5) {
            ...
            if (ext_lba_base != 0) {
                ...
                // ext_lba_base为0表示是第一次读取引导块 也就是主引导分区记录所在的扇区
                // 记录下拓展的分区的起始lba地址 后面所有的拓展分区都相对于此
...
static bool partition_info(struct list_elem* pelem, int arg UNUSED) {
    ...
    // 此处return false和函数本身功能无关
    // 为了让主函数调用list_traversal继续向下遍历元素
    return false;
...
void ide_init() {
    ...
    while (channel_no < channel_cnt) {
        ...
        sema_init(&channel->disk_done, 0);                          // 下一个channel;
