...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread, struct task_struct* parent_thread) {
    ...
    // a 复制pcb所在的整个页 里面包括进程pcb的信息及特权0级的栈 里面包含了返回地址
    memcpy(child_thread, parent_thread, PG_SIZE);
    // 下面单独修改
    child_thread->pid = fork_pid();
    child_thread->elapsed_ticks = 0;
    child_thread->status = TASK_READY;
    child_thread->ticks = child_thread->priority;       // 为新进程把时间片填满
    ...
    // 下面将指向自己的位图vaddr_btmp
    memcpy(vaddr_btmp, child_thread->userprog_vaddr.vaddr_bitmap.bits, bitmap_pg_cnt * PG_SIZE);
    child_thread->userprog_vaddr.vaddr_bitmap.bits = vaddr_btmp;
    
    // debug
    ASSERT(strlen(child_thread->name) < 11);        // pcb.name 的长度是6 避免下面strcat越界
        ...
        if (vaddr_btmp[idx_byte]) {
            ...
            while (idx_bit < 8) {
                ...
                if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte]) {
                    ...
                    // 下面操作是将父进程用户空间中的数据通过内核空间做中专 最终复制到子进程的用户空间
                    // a 将父进程在用户空间中的数据复制到内核缓冲区buf_page目的是下面切换到紫禁城的页表后还能访问父进程的数据
                    memcpy(buf_page, (void*)prog_vaddr, PG_SIZE);
                    // b 将页表切换到子进程 目的是避免下面申请内存的函数将pte以及pde安装在父进程的页表中
...
static int32_t build_child_stack(struct task_struct* child_thread) {
    ...
    // swtich_to的返回地址更新为intr_exit 直接从中断返回
    *ret_addr_in_thread_stack = (uint32_t)intr_exit;

    // 下面两行是为了让构建的thread_stack更加清晰 其实也不太需要
...
static int32_t copy_process(struct task_struct* child_thread, struct task_struct* parent_thread) {
    ...
    // a 复制父进程的pcb虚拟位图 内核子进程
    if (copy_pcb_vaddrbitmap_stack0(child_thread, parent_thread) == -1) {

         return -1;

    }
    // b 为子进程创建页表 此页表项包括内核空间
