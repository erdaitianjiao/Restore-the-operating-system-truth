...
// 激活线程或进程的页表 更新tss中的esp0为进程特权级为0的栈
void process_activate(struct task_struct* p_thread) {
    ...
    // 内核线程特权本身就是0 处理器进入中断时不会从tss中获取0级特权栈地址 所以不用更新esp0
    if (p_thread->pgdir) {
...
// 创建页目录页 将当前页表的表示内核空间的pde复制
// 成功则返回目录页的虚拟地址 否则返回-1
uint32_t* create_page_dir(void) {

    // 用户进程的页表不能直接让用户访问到 所以在内核空间来申请
    ...
    // 先复制页表 page_dir_vaddr + 0x300 * 4 是内核页目录的第768项
